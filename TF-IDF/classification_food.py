...
N_MODEL = 4 # Edit here
CATEGORY = ['Airline','Accommodation','Tourism','Restaurant & Delivery'] # Edit here
N_SUBMODEL_RESTAURANT = 4
SUBMODEL_RESTAURANT = ['BarbequeGrill','Japanese','Seafood','SukiyakiShabu']

known_category_phone_number = {}
new_format_input_path = './new_number_input.txt'
unknown_number_input_path = './unknown_number_input.txt'

PASS_RATE = 0.5 #Edit here
...
def predict(filename_in='./number_input.txt',filename_out='./results/result.csv'):
    ...
    for model_id in range(1,N_MODEL+1,1):        
        ...
        for row_i in range(len(mobile_no)):
            print "Mobile no {:12} ==> Prob of this class: {}".format(mobile_no[row_i],proba_list[row_i][1])
            result[mobile_no[row_i]].append(proba_list[row_i][1])
    
    # Each Restaurant Subcategory model
    for model_id in range(1,N_SUBMODEL_RESTAURANT+1,1):        
        # Load Classification Model
        model_file_name = 'sklearn_sub_restaurant_model_%02d.pickle' % model_id
        model_file_path = os.path.join(MODEL_DIR_PATH, model_file_name)
        print "Restaurant Subcat Model#%d:" % model_id, model_file_path
        classifier = load_model(model_file_path)
        ...
        for row_i in range(len(mobile_no)):
            # is not Restaurant
            if result[mobile_no[row_i]][3]<PASS_RATE:
                print "Mobile no {:12} ==> NOT RESTAURANT".format(mobile_no[row_i],proba_list[row_i][1])
                result[mobile_no[row_i]].append(0)
            else:
                print "Mobile no {:12} ==> Prob of this class: {}".format(mobile_no[row_i],proba_list[row_i][1])
                result[mobile_no[row_i]].append(proba_list[row_i][1])
    ...
    with open(filename_out, 'w') as file_out:
        for cat in CATEGORY:
            file_out.write(','+cat)
        for cat in SUBMODEL_RESTAURANT:
            file_out.write(','+cat)
        for cat in CATEGORY:
            file_out.write(',subcat_'+cat)
        file_out.write(',other,\n')
        key_str_list = result.keys()
        value_str_list = [','.join(map(lambda n: "{:.6f}".format(n),result_row)) for result_row in result.values()]
        ...
        for num_key in phone_cat:
            ...
            # SUBCAT RESTAURANT (EDIT HERE)
            # 'BarbequeGrill','Japanese','Seafood','Sukiyaki/Shabu'
            file_out.write( \
            ('1' if 'Barbeque/Grill' in phone_cat[num_key]['restaurant'] else '0') + ',' + \
            ('1' if 'Japanese' in phone_cat[num_key]['restaurant'] else '0') + ',' + \
            ('1' if 'Seafood' in phone_cat[num_key]['restaurant'] else '0') + ',' + \
            ('1' if 'Sukiyaki/Shabu' in phone_cat[num_key]['restaurant'] else '0') + ',' )
    ...
    print "Success :: Result is saved ({})!".format(filename_out)
